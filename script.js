// script.js - Updated for dropdown fixes and Try Now improvements
...
// Tool configurations for Try Now section
const tools = {
    'color-picker': {
        title: 'Color Picker',
        ...
        content: `
            <div class="color-picker-container">
                <div id="colorDisplay" class="color-display" style="background-color: #8a2be2;"></div>
                <div class="color-inputs">
                    <input type="color" id="colorPicker" value="#8a2be2" title="Choose a color">
                    <input type="text" id="hexInput" placeholder="#8a2be2" maxlength="7">
                    <input type="text" id="rgbInput" placeholder="rgb(138, 43, 226)">
                </div>
                <div id="colorValue" class="color-value">#8a2be2 | rgb(138, 43, 226)</div>
                <button id="copyColor" class="try-now-btn" style="margin-top: 1rem;">
...
function init() {
    ...
    // Check if modal exists and setup modal
    if (modal && closeModal) {
...
// ===== TRY NOW TOOLS SETUP =====
function setupTryNowTools() {
    // Add event listeners to Try Now buttons
    document.querySelectorAll('.try-now-btn').forEach(button => {
        button.addEventListener('click', function(e) {
            // Only handle if it's not a link to another page
            if (!this.getAttribute('href') || this.getAttribute('href') === '#') {
                e.preventDefault();
                const toolName = this.textContent.trim();
                
                // For now, only handle Color Picker
                if (toolName === 'Try Now' && this.closest('.try-now-card')) {
                    const card = this.closest('.try-now-card');
                    const toolTitle = card.querySelector('h3').textContent;
                    
                    if (toolTitle === 'Color Picker') {
                        openToolModal('color-picker');
                    } else {
                        showToast(`The ${toolTitle} tool is coming soon!`);
                    }
                }
            }
        });
...
function initializeColorPickerTool() {
    ...
    if (!colorPicker || !hexInput || !rgbInput || !colorDisplay || !colorValue || !copyBtn) return;
    ...
    if (copyBtn) {
        copyBtn.addEventListener('click', () => {
            ...
            navigator.clipboard.writeText(text).then(() => {
                ...
                showToast('Failed to copy color. Please try again.');
            });
        });
    }
